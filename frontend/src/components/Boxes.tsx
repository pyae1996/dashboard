        ...
        <div className="h-[50vh]">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Area
              name="Accumulated Boxes"
                type="monotone"
                dataKey="accumulating_total_tasks"
                fill="#8884d8"
                stroke="#8884d8"
              />
              <Bar name="Boxes" dataKey="total_tasks" barSize={20} fill="#413ea0" />
              {/* @ts-ignore */}
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-2 border border-gray-200 shadow rounded">
        ...
        <p>Accum Boxes: {payload[0].payload.accumulating_total_tasks}</p>
        <p>Boxes: {payload[0].payload.total_tasks}</p>
        <p>Successful Picks: {payload[0].payload.total_successful_pick_count}</p>
      </div>
    );
  }
